...
class PlanetNode: SCNNode {
    ...
    init(planet: PlanetEnum) {
        ...
        let geometry = SCNSphere(radius: planetData.radius)
        geometry.firstMaterial?.diffuse.contents = planetData.diffuse
        geometry.firstMaterial?.specular.contents = planetData.specular
        geometry.firstMaterial?.emission.contents = planetData.emission
        geometry.firstMaterial?.normal.contents = planetData.normal
    ...
    func setPosition(_ newPosition: SIMD3<Float>, relativeTo cameraTransform: matrix_float4x4, smoothMovement: Bool) {
        ...
        if smoothMovement {
            ...
            guard let averageDistance = recentVirtualObjectDistances.average else { return }
    ...
    func addPlanet(planet: PlanetEnum) {
        ...
        let orbitNode = self.getPlanetOrbit(planet: planet.getPlanet())
    ...
    private func getPlanetOrbit(planet: SolarPlanet) -> SCNNode {
        ...
        oribitNode.geometry?.firstMaterial?.multiply.contents = UIColor(red: 255, green: 255, blue: 255, alpha: 0.1)
