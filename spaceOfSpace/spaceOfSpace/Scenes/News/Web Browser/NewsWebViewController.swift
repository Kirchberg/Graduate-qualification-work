//
//  NewsWebBrowserViewController.swift
//  spaceOfSpace
//
...
protocol NewsWebBrowserDisplayLogic: AnyObject {
    func displayWebBrowser(viewModel: NewsWebBrowser.showShareMenu.ViewModel)
}

class NewsWebBrowserViewController: VerticalViewController, NewsWebBrowserDisplayLogic {
    var interactor: NewsWebBrowserBusinessLogic?
    var router: (NewsWebBrowserRoutingLogic & NewsWebBrowserDataPassing)?
    ...
    func displayWebBrowser(viewModel: NewsWebBrowser.showShareMenu.ViewModel) {
        urlToShow = viewModel.hyperlink
        if let urlToShow = urlToShow {
            let myRequest = URLRequest(url: urlToShow)
            webView.load(myRequest)
        }
    }
}

extension NewsWebBrowserViewController: WKUIDelegate, WKNavigationDelegate {
